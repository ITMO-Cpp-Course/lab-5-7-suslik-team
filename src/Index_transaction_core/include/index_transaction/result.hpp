...
inline std::string errorCodeToString(ErrorCode code)
{
    ...
    case ErrorCode::InvalidWord:
        return "Invalid word in document";
...
template <typename T> struct ResultImpl
{
    using type = std::expected<T, ErrorCode>;
};

template <> struct ResultImpl<void>
{
    using type = std::expected<void, ErrorCode>;
};

template <typename T> using Result = typename ResultImpl<T>::type;
